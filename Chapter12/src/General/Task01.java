...
public class Task01 {
    public static void main(String[] args) {
        HashMap<Integer, List<String>> allCity = new LinkedHashMap<>();
        HashMap<Integer, String> allProvinces = new LinkedHashMap<>();
        ...
        for (int i = 0 ; i < provinces.length; i++) {
            String[] temp = provinces[i].split(":"); // 根据":"分割省份和城市
            String province = temp[0].replace("'", ""); // 获得省份名称
            allProvinces.put(i, province);

            List<String> cityList = new ArrayList<>(); // 用于存储城市
            String[] city = temp[1].replaceAll("\\[|\\]|\\'| ", "").split(",");
            Collections.addAll(cityList, city); // 一个省份的所有城市存入一个ArrayList中
            allCity.put(i, cityList);
        }

        Set<Map.Entry<Integer, String>> set = allProvinces.entrySet();
        for (Object o : set) {
        ...
        System.out.println(allCity.get(num));
    }
}
